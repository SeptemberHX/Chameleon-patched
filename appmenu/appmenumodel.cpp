...
{
    qCDebug(category) << id << "AppMenuModel::onActiveWindowChanged starts";
    qApp->removeNativeEventFilter(this);

    if (m_winId!=-1  && m_winId!=id) {
        //! 考虑到 Decoration 只需要关注该窗口相关内容，这里屏蔽掉所有其他的窗口变化
        qCDebug(category) << id << "AppMenuModel::onActiveWindowChanged ends because m_winId != id";
        return;
    }

    if (!id) {
        qCDebug(category) << id << "AppMenuModel::onActiveWindowChanged ends because id == 0";
        setMenuAvailable(false);
        emit modelNeedsUpdate();
        return;
    }
    ...
    if (KWindowSystem::isPlatformX11()) {
        ...
        // KDE 将每个窗口的 global menu 的 dbus 服务名称、路径存到了每个窗口的属性里。下面两个函数就是用来读取这两个属性的。
        ...
        auto updateMenuFromWindowIfHasMenu = [this, &getWindowPropertyString](WId id) {
            ...
            qCDebug(category) << id << "AppMenuModel::updateMenuFromWindowIfHasMenu starts" << serviceName << menuObjectPath;
...
{
    qCDebug(category) << this->m_winId << "AppMenuModel::updateApplicationMenu starts" << serviceName << menuObjectPath;
    ...
    qCDebug(category) << this->m_winId << "AppMenuModel::updateApplicationMenu ends 2";
}
