...
static void
setWindowProperty(xcb_window_t WId, xcb_atom_t propAtom, xcb_atom_t typeAtom, int format, const QByteArray &data) {
    if (!isPlatformX11())
        return;
    ...
    if (format == 0 && data.isEmpty()) {
        ...
        xcb_change_property(conn, XCB_PROP_MODE_REPLACE, WId, propAtom, typeAtom, format, data.length() * 8 / format,
                            data.constData());
    }
}
...
public:
    KWinInterface() {
        ...
        quickTileWindow = (QuickTileWindow) KWinUtils::resolve(
                "_ZN4KWin9Workspace15quickTileWindowE6QFlagsINS_13QuickTileFlagEE");
        x11CursorGetCursor = (X11CursorGetCursor) KWinUtils::resolve("_ZN4KWin6Cursor9x11CursorEN2Qt11CursorShapeE");
        optionsWindowOperation = (OptionsWindowOperation) KWinUtils::resolve(
                "_ZN4KWin7Options15windowOperationERK7QStringb");
        ...
        findUnmanagedByFunction = (WorkspaceFindUnmanagedByFunction) KWinUtils::resolve(
                "_ZNK4KWin9Workspace13findUnmanagedESt8functionIFbPKNS_9UnmanagedEEE");
        sendPingToWindow = (WorkspaceSendPingToWindow) KWinUtils::resolve("_ZN4KWin9Workspace16sendPingToWindowEjj");
        xcbExtensionsShapeNotifyEvent = (XcbExtensionsShapeNotifyEvent) KWinUtils::resolve(
                "_ZNK4KWin3Xcb10Extensions16shapeNotifyEventEv");
        ...
        if (!compositorSuspend) {
            compositorSuspend = (CompositorToggle) KWinUtils::resolve(
                    "_ZN4KWin13X11Compositor7suspendENS0_13SuspendReasonE");
        }

        compositorResume = (CompositorToggle) KWinUtils::resolve("_ZN4KWin10Compositor6resumeENS0_13SuspendReasonE");
        if (!compositorResume) {
            compositorResume = (CompositorToggle) KWinUtils::resolve(
                    "_ZN4KWin13X11Compositor6resumeENS0_13SuspendReasonE");
        }
...
KWinUtils::KWinUtils(QObject *parent)
        : QObject(parent), d(new KWinUtilsPrivate(this)) {
#ifdef KWIN_VERSION
...
QObjectList KWinUtils::clientList() {
    ...
    QObject *jsWorkspaceWrapper = KWinUtils::findObjectByClassName(QByteArrayLiteral("KWin::QtScriptWorkspaceWrapper"),
                                                                   scripting_children);
...
void
KWinUtils::setWindowProperty(const QObject *client, quint32 atom, quint32 type, int format, const QByteArray &data) {
    bool ok = false;
    xcb_window_t wid = getWindowId(client, &ok);
...
void KWinUtils::Window::performWindowOperation(QObject *window, const QString &opName, bool restricted) {
    ...
    QMetaObject::invokeMethod(workspace(), "performWindowOperation", Q_ARG(KWin::AbstractClient*, c),
                              Q_ARG(KWin::Options::WindowOperation, op));
}
